...
//
// Usage:
//     ./TestFEMPoissonSolver_gaussian2d --info 5
...
template <typename T>
KOKKOS_INLINE_FUNCTION T gaussian2D(ippl::Vector<T, 2> x_vec, const T& sigma = 0.05, const T& mu = 0.5) {
...
struct AnalyticSol {
    KOKKOS_FUNCTION const T operator()(ippl::Vector<T, 2> x_vec) const {
        ...
        return ((r2 / sigma*sigma*sigma*sigma) - (2.0 / sigma*sigma)) *
...
void testFEMSolver(const unsigned& numNodesPerDim, const T& domain_start = 0.0,
    ...
    ippl::parallel_for(
        "Assign RHS", rhs.getFieldRangePolicy(), KOKKOS_LAMBDA(const index_array_type& args) {
            ...
            apply(view_rhs, args) = gaussian2D<T>(x);
