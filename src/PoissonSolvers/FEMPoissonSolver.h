    ...
    struct EvalFunctor {
        ...
        KOKKOS_FUNCTION const auto operator()(
            ...
            const Vector<Vector<Tlhs, Dim>, numElemDOFs>& grad_b_q_k) const {
                return (((DPhiInvT * grad_b_q_k[j])[0]*(DPhiInvT * grad_b_q_k[i])[0]) + ((DPhiInvT * grad_b_q_k[j])[1]*(DPhiInvT * grad_b_q_k[i])[1]) + ((DPhiInvT * grad_b_q_k[j])[2]*(DPhiInvT * grad_b_q_k[i])[2])) * absDetDPhi;
